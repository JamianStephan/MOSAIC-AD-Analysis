...
def calculate_FWHM(wavelength,airmass,median_FWHM,median_FWHM_lambda,kolb_factor=True):
    ...
    if kolb_factor==True: #FWHM using kolb factor; smaller FWHM
        ...
        FWHM_atm=median_FWHM.to(u.arcsec).value*airmass**(0.6)*(wavelength.to(u.nm).value/median_FWHM_lambda.to(u.nm).value)**(-0.2)*np.sqrt(1+F_kolb*2.183*(r0/L0)**0.356)
        FWHM_DL=0.000212*wavelength.to(u.nm).value/D
        FWHM=np.sqrt(FWHM_atm**2+FWHM_DL**2)
...
def numerical_moffat(aperture,FWHM,offset,scale,beta=2.5):
    ...
    x, y = np.meshgrid(x, y)
    Moffat=Moffat2D(1,abs(x_pos.to(u.arcsec).value),0,alpha,beta)
