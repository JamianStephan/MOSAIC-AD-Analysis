...
def analytical_gaussian(aperture_diameter,FWHMs,shifts,k_lim):
    ...
    """
    aperture_radius=aperture_diameter.value/2
    offset=shifts.value
    FWHMs=FWHMs.value
...
def numerical_durham(aperture,PSF,offset,scale,axis_val=24,data_version=0):
    ...
    durham_data=durham_data[int(data_boundary/2-aperture_boundary):int(data_boundary/2+aperture_boundary)+1,int(data_boundary/2-aperture_boundary+offset.value/scale):int(data_boundary/2+aperture_boundary+offset.value/scale)+1]
