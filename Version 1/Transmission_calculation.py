...
def numerical_fits(scale,offset,unknown=0):
    #fits file in array form, with a given scale
    #downsample scale through binning, and save new scale: take mean?
    #take a section of the array, a boundaryxboundary section, where the aperture will be
    #see notes, PSF_array[a-d-boundary:a-d+boundary+1][a-boundary:a+boundary+1], a = round(len(PSF_array)/2)
    #convolved aperture with boundaryxboundary section
    #trans = sum of convolved data vals / sum of fits file pixel vals
    trans=unknown
    return trans
